...
function PhaseOnePanel({
  ...
  const yieldMetric = (metrics: Metric[], label: string) => {
    const repeater = metrics.find((metric) => metric.name === "Organic second-situation rate");
    const cohort = metrics.find((metric) => metric.name === "Eligible cohort completed")?.actual
      ?? metrics.find((metric) => metric.name === "Analytics reconciliation")?.actualDenominator
      ?? null;
    const count = repeater?.actual ?? null;
    const percent = count !== null && cohort !== null && cohort > 0 ? Math.round((count / cohort) * 100) : null;
    return <article className="phase1-yield"><p className="eyebrow">HEADLINE TRACTION METRIC</p><h3>{label}</h3><b>{count === null || cohort === null ? "—" : `${count} / ${cohort}`} {percent === null ? "" : `= ${percent}%`}</b><span>Organic second-situation users / all eligible users invited. Not an additional advancement gate.</span></article>;
  };
  const wedges = [

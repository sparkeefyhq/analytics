...
function ControlSidebar({ view, setView, email, onLogout, onLogin }: { view: ControlView; setView: (view: ControlView) => void; email: string | null; onLogout: () => void; onLogin: () => void }) {
  return <aside className="control-sidebar">
    ...
    <div className="control-sidebar-foot"><span className="status-dot" />{email ? "Secure internal workspace" : "Public preview"}<small>{email || "View-only"}</small>{email ? <button onClick={onLogout}>Sign out</button> : <button onClick={onLogin}>Sign in to edit</button>}</div>
  </aside>;
}
...
function PlanPage({ tracker, save, onLogin }: { onLogin: () => void; tracker: ControlTracker; save: (action: string, patch?: Record<string, unknown>, id?: string) => Promise<void> }) {
  ...
  return <section className="control-page plan-page">
    <header className="control-header"><h1>Plan</h1><div className="phase-header-controls"><PhaseClock phase={phase} />{current?.state === "ready" ? <button className="phase-status ready" disabled={starting} onClick={() => tracker.canEdit ? void startPhase() : onLogin()}>{starting ? "Starting…" : "Ready"}</button> : <span className={`phase-status ${current?.state}`}>{current?.state === "active" ? "Active" : current?.state === "complete" ? "Completed" : "Locked"}</span>}</div></header>
